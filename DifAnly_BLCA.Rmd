...
#### This R Markdown file performs miRNA differential expression analysis for the Bladder Tissue Samples queried from the GDC API. Expression analysis is done with the edgeR and limma packages.
...
raw_rd <- read.csv("../../CleanData/BLCA_rds.csv")
...
```{r}
groups_samp <- append(
  rep(c("T"),length(colnames(tumor_t))),
  rep(c("N"),length(colnames(normal_t))))
```
...
#write.csv(temp, "../../CleanData/BLCA_DifExp_outPut.csv")
```
